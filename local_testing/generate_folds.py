...
train_files = train_files.rename(columns={'_id': 'id'})
...
for i, (train_indices, test_indices) in enumerate(folds):
    ...
    with open(os.path.join(FOLD_INFO_PATH, f"fold_{i}", "train.csv"), 'w') as train_fold_info:
        train_files.iloc[train_indices].set_index('id').to_csv(train_fold_info)
    with open(os.path.join(FOLD_INFO_PATH, f"fold_{i}", "test.csv"), 'w') as test_fold_info:
        train_files.iloc[test_indices]['id'].to_csv(test_fold_info, index=False)
    with open(os.path.join(FOLD_INFO_PATH, f"fold_{i}", "test_answer.csv"), 'w') as test_fold_info:
        train_files.iloc[test_indices].set_index('id').to_csv(test_fold_info)
